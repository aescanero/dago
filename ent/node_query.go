...
func (_q *NodeQuery) QueryGraph() *GraphQuery {
...
func (_q *NodeQuery) WithGraph(opts ...func(*GraphQuery)) *NodeQuery {
	query := (&GraphClient{config: _q.config}).Query()
...
func (_q *NodeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Node, error) {
	...
	if query := _q.withGraph; query != nil {
		if err := _q.loadGraph(ctx, query, nodes, nil,
			func(n *Node, e *Graph) { n.Edges.Graph = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *NodeQuery) loadGraph(ctx context.Context, query *GraphQuery, nodes []*Node, init func(*Node), assign func(*Node, *Graph)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Node)
	for i := range nodes {
		if nodes[i].graph_nodes == nil {
			continue
		}
		fk := *nodes[i].graph_nodes
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "graph_nodes" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
