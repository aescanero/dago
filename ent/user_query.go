...
// QueryOrgUnit chains the current query on the "org_unit" edge.
func (_q *UserQuery) QueryOrgUnit() *OrgUnitQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withOrgUnit: _q.withOrgUnit.Clone(),
...
// WithOrgUnit tells the query-builder to eager-load the nodes that are connected to
// the "org_unit" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithOrgUnit(opts ...func(*OrgUnitQuery)) *UserQuery {
	query := (&OrgUnitClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withOrgUnit = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withOrgUnit; query != nil {
		if err := _q.loadOrgUnit(ctx, query, nodes, nil,
			func(n *User, e *OrgUnit) { n.Edges.OrgUnit = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadOrgUnit(ctx context.Context, query *OrgUnitQuery, nodes []*User, init func(*User), assign func(*User, *OrgUnit)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*User)
	for i := range nodes {
		if nodes[i].org_unit_users == nil {
			continue
		}
		fk := *nodes[i].org_unit_users
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "org_unit_users" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
